...
typedef struct
{
	...
	double psi;            /* (MPa) water potential of soil */
	double leaf_psi;	/* (MPa) water potential of leaves added JDW 09/24/2018 */
	...
	double m_psi;		/* (DIM) leaf water potential multiplier */
	...
	double m_Kl;           /*  (DIM) leaf hydraulic conductivity multiplier added JDW 03/16/2018 */
	double m_psi_x;           /*  (DIM) stem hydraulic conductivity multiplier  WJM 0821 */
...
//	bool allow_refill;        /* (bool) flag to enforce refilling threshold*/

	/* diagnostic variables for ouput purposes only */
	double ytd_maxplai;    /* (DIM) year-to-date maximum projected LAI */

	/* new carbon isotope calculations */
	double d13C_leaf_sun; /*(‰) carbon isotope discrimination of sun leaves */
	double d13C_leaf_shade; /*(‰) carbon isotope discrimination of shade leaves */
...
typedef struct
{
    ...
    double max_mesophyll_path; /* (um) maximum length from mesophyll to stomata */
    double min_mesophyll_path; /* (um) maximum length from mesophyll to stomata */
	/*WJM 0821*/
    double psi_p_50; /* (MPa) xylem water potential at which PLC is 0.5 */
    double s_psi_p_50; /* (MPa) standard deviation of normally distributed PLC curve */
    double psi_refill; /* (MPa) standard deviation of normally distributed PLC curve */
...
typedef struct
{
	...
	double vegn;           /* kgN/m2  total vegetation N*/
        double litrn;          /* kgNm2  total litter N */
	double soiln;          /* kgN/m2  total soil N */
	double totaln;         /* kgN/m2  total of vegN, litrN, and soilN */
} summary_struct;
