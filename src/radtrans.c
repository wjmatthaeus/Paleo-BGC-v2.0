...
metvar_struct* metv, epvar_struct* epv, psn_struct *psn_sun, psn_struct *psn_shade, double albedo)/*WJM 0921*/
{
	...
	*/
	...
	if (cs->leafc > 0.0)
	{
		...
		divide by leafc, but no incoming test for very low leafc WJM 9021*/
		epv->sun_proj_sla = (epv->plaisun + (epv->plaishade/epc->sla_ratio)) /
			cs->leafc;
		epv->shade_proj_sla = epv->sun_proj_sla * epc->sla_ratio;
		
	}
	else if (cs->leafc == 0.0)/*works because of precision_control.c*/
	{
		epv->all_lai = 0.0;
		epv->proj_lai = 0.0;
		epv->plaisun = 0.0;
		...
		/*canopy_et.c and photosynthesis.c don't get called when cs.leafc is (close to) zero 
		update them here WJM 0921*/
		epv->gl_bl		= 0.0;
		epv->gl_c		= 0.0;
		epv->gl_s_sun	= 0.0;
		epv->gl_s_shade	= 0.0;
	...
	else
	{
		...
		/*WJM 0821*/
                bgc_printf(BV_ERROR, "epc->lai_ratio = %.7e\n",epc->lai_ratio);
                bgc_printf(BV_ERROR, "epc->max_mesophyll_path = %.7e\n",epc->max_mesophyll_path);
		ok=0;
	}
