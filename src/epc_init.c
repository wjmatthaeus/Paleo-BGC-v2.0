...
int epc_init(file init, epconst_struct* epc)
{
	...
	********************************************************************/
	...
	if (ok && scan_value(temp, &epc->max_mesophyll_path, 'd'))
	{
		bgc_printf(BV_ERROR, "Error reading max_mesophyll_path, epc_init()\n");
		ok=0;
	}
	if (ok && scan_value(temp, &epc->min_mesophyll_path, 'd'))
	{
		bgc_printf(BV_ERROR, "Error reading min_mesophyll_path,epc_init()\n");
		ok=0;
	}
	/*WJM 0821*/
	if (ok && scan_value(temp, &epc->psi_p_50, 'd'))
	{
		bgc_printf(BV_ERROR, "Error reading psi_p_50, epc_init()\n");
		ok=0;
	}
	if (ok && scan_value(temp, &epc->s_psi_p_50, 'd'))
	{
		bgc_printf(BV_ERROR, "Error reading s_psi_p_50,epc_init()\n");
		ok=0;
	}
		if (ok && scan_value(temp, &epc->psi_refill, 'd'))
	{
		bgc_printf(BV_ERROR, "Error reading psi_refill,epc_init()\n");
		ok=0;
	}
	fclose(temp.ptr);
